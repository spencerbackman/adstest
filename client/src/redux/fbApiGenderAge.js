...
const insightsFields =
  ['impressions', 'frequency', 'clicks', 'ctr', 'ad_name', 'adset_name'];

  export function getFbApiGenderAge() {
      return dispatch => {
        ...
        .then((insights) => {
            ...
            dispatch({
                type: "GET_FB_API_GENDER_AGE",
                fbApiGenderAge: insights
            })
        }).catch(err => console.log(err));
      }
  }

export default function reducer(fbApiGenderAge = [], action) {
    switch(action.type) {
        case 'GET_FB_API_GENDER_AGE':
            return action.fbApiGenderAge;
        default: 
            return fbApiGenderAge;
    }
}